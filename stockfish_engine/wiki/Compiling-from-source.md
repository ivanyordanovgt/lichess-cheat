...
make strip
mv stockfish_engine.exe ../../stockfish_${arch_cpu}.exe
make clean
cd
```
</details>
...
```bash
$ mkdir -p ~/repos/official-stockfish_engine
$ cd ~/repos/official-stockfish_engine
...
```bash
$ file stockfish_engine
stockfish_engine: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, with debug_info, not stripped
```
...
aarch64-linux-android-strip stockfish_engine
```
...
```bash
$ file stockfish_engine
stockfish_engine: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, stripped
```

It is a good idea to rename the binary:

```bash
$ mv stockfish_engine stockfish_DEV_armv8
```
...
mv stockfish_engine.exe ../../stockfish_engine-x64${1}.exe
...
make strip COMP=mingw
mv stockfish_engine.exe ../../stockfish_engine-x64${1}-pgo.exe
make clean COMP=mingw
}
...
qemu-aarch64 stockfish_engine compiler
qemu-aarch64 stockfish_engine bench
...
# test: qemu's magic at work
qemu-arm stockfish_engine compiler
qemu-arm stockfish_engine bench
...
cd Stockfish/src
make -j build ARCH=armv8 COMP=gcc CXX="zig c++ -target aarch64-linux-musl"
mv stockfish_engine.exe stockfish_armv8
```
</details>
